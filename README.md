...
Animated grid RTL layout component for React.
Choose between CSS Transitions or [React-Motion](https://github.com/chenglou/react-motion) for animation.

[Demo](https://git.socialsky.io:socialsky/react-rtl-grid.git)  

## Installation

With [npm](git+ssh://git@git.socialsky.io:socialsky/react-rtl-grid.git):

```
npm install --save git+ssh://git@git.socialsky.io:socialsky/react-rtl-grid.git
```
...
```js
import { SpringGrid } from 'react-rtl-grid';
```
```xml
<SpringGrid
  component="ul"
...
```js
import { CSSGrid, layout } from 'react-rtl-grid';
```
```xml
<CSSGrid
  component="ul"
...
```js
import { SpringGrid, measureItems } from 'react-rtl-grid';
...
```js
import { CSSGrid, measureItems, makeResponsive } from 'react-rtl-grid';
...
Included layouts:
```js
import { layout } from 'react-rtl-grid';
...
```js
import { enterExitStyle } from 'react-rtl-grid';

const { enter, entered, exit } = enterExitStyle.foldUp;
```
Check out the [demo](http://git.socialsky.io:socialsky/react-rtl-grid.git) to see them in action.
...
```js
import { easings } from 'react-rtl-grid';
